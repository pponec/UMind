...
| File | What it is for |
|---|---|
| `06-readme-example.json` | The showcase map pictured in the project README (`docs/images/graph-example.png`): a small, ordinary plan whose descriptions happen to use a table, a numbered list, a block quote, bold, italics and a link. Re-shoot the screenshot from its graph view (`?weekend-trip/graph` once opened and saved) if the drawing style changes. |
